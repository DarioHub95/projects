...
for ((i=1; i<=$1; i++)); do
    ...
    count=0
    while [ $count -eq 0 ]; do
        srun --job-name="$job_name_J${i}" -p parallel -n $num_tasks a.out > srun.log 2>&1 &
        sleep 1

        # Verifica dello stato del job i-esimo
        job_id=$(squeue -u $USER -n "$job_name_J${i}" -o "%i" -h | head -n 1)
        ...
        if [[ "$job_status" == "PD" ]]; then
            echo "Il job $job_name_J${i} non è riuscito a partire poichè in pending..."
            echo "Cancellazione del job..."
            scancel $job_id
            echo "Riduzione del numero di task di 10."
            ((num_tasks -= 10))
            if (( $num_tasks < 10 || $num_tasks < 0 )); then
                echo "Il numero di task è inferiore a 10 o <0. Cancellazione del job $job_name_J${i}..."
                ((count++))
                scancel $job_id
