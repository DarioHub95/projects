...
// appende il parametro (se non esiste già) alla lista s_argc,s_argv, altrimenti lo ignora...
static void append_args(const char *form,...)
  {
   ...
   char *p=buffer+n;
   char *q=strchr(p,'=');
   if (q==NULL) return;			// il formato non è corretto...
   *q='\0';
   q++;
   char *s=q;
   while (*s)
   ...
   if (i==s_argc)								// il parametro non esiste... appendilo alla lista
     {
      ...
      if (pvm_debug()==0)			// scrivi in getarg.log
        {
         static int count;
      ...
      s_flag[s_argc]=(n>0?0:1);			// indica se deve essere scritto in stdout
      s_argc++;
     }
  }

// legge la linea nline del file di dati "path" (la prima linea corrisponde a nline=0)
// se nline è maggiore del numero di linee, legge la linea nline%nmax, dove nmax è il numero di linee
// interpreta i dati della linea come i parametri elencati nella stringa "name", ad esempio name="alpha beta gamma"
// appende i parametri alla lista s_argc, s_argv...
int getlinearg(const char *path,int nline,const char *name)
  {
   ...
   fclose(stream);
   char *p=buffer;              // linea di indice nline&nmax (la prima linea corrisponde a nline=0) del file di dati
   char buff2[PATH_MAX];
   strcpy(buff2,name);
   char *q=buff2;
   while (1)
   ...
   for (int i=1;i<a_argc;i++)
     {
      ...
      //printf("append (cmd line): %s\n",p);
      append_args("%s",p);
     }
   // ARGOMENTI DAL FILE SEED
