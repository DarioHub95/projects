...
// da usare al posto di cputerm all'interno di cicli in cui si scambiano messaggi...
// restituisce lo stesso valore per tutti i processori
// evita che qualche processo esca dal ciclo e qualche altro no...
// N.B. non eseguire "if (cpuint() || mpi_term())" altrimenti dove cpuint()==0 viene chiamato, dove cpuint()==1 no
// e nemmeno "if (mpi_term() || cpuint())" se all'interno dell'if si deve usare il risultato di mpi_term()
// eseguire invece "int flag=mpi_term()" e poi usare il valore di flag
int mpi_term(void)
  {
   int mype=0;
#ifdef __USE_MPI__
